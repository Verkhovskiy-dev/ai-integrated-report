      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        {["upper", "middle", "lower"].map((group) => (
          <div key={group} className="flex items-center gap-2">
            ...
            />
            <span className="text-xs text-muted-foreground">{getGroupLabel(group)}</span>
          </div>
        ))}
        <div className="flex items-center gap-1 ml-4">
          <span className="text-xs text-muted-foreground mr-1">Интенсивность:</span>
          {[0, 1, 2, 3, 4].map((v) => (
            <div
              key={v}
          ...
          <span className="text-[10px] text-muted-foreground ml-1">0 — 4+</span>
        </div>
      </div>
        ...
        <div className="min-w-[700px]">
          ...
          {SRT_LEVELS.map((level) => (
            <div key={level.id} className="flex items-center group">
              <div className="w-48 shrink-0 flex items-center gap-2 py-1 pr-3">
                <div
                  className="w-1.5 h-6 rounded-full shrink-0"
                  style={{ backgroundColor: getGroupColor(level.group) }}
                />
                <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">
                  <span className="font-mono text-foreground/60 mr-1">{level.id}</span>
                  {level.short}
                </span>
              </div>
              {HEATMAP_DATA.map((d) => {
                  ...
                  <Tooltip key={d.date}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <div
                          className={`
                            w-full h-7 rounded-sm transition-all duration-200 border
