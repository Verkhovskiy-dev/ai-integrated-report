        ...
        <div className="relative z-10 p-4 sm:p-6 lg:p-10">
          ...
          <div className="space-y-3 sm:space-y-4">
            {CROSS_LEVEL_CONNECTIONS.map((conn) => (
              <div
                key={conn.id}
                id={buildShareId("connection", conn.id)}
                className="relative bg-card/40 backdrop-blur-sm border border-border/40 rounded-lg p-3 pr-12 sm:p-4 sm:pr-12 hover:border-primary/20 transition-all duration-300"
              >
                <ShareButton id={buildShareId("connection", conn.id)} title={conn.title} compact className="absolute right-2 top-2" />
