          ...
          <section className="space-y-4">
            ...
            <div className="overflow-x-auto rounded border">
              <table className="w-full min-w-[760px] text-left text-sm">
                <thead className="bg-muted/30"><tr><th className="p-3">Новость / версия</th><th className="p-3">Полезно</th><th className="p-3">Важно</th><th className="p-3">Подробнее</th><th className="p-3">Неясно</th><th className="p-3">Всего активно</th></tr></thead>
                <tbody>{summary.newsVersions.map((row) => <tr key={`${row.newsId}:${row.contentVersion}`} className="border-t"><td className="p-3"><a href={row.url} className="font-medium text-primary underline" target="_blank" rel="noreferrer">{row.title}</a><div className="mt-1 font-mono text-[10px] text-muted-foreground">{row.newsId} · {row.contentVersion}</div></td><td className="p-3">{row.useful}</td><td className="p-3">{row.important ?? 0}</td><td className="p-3">{row.more}</td><td className="p-3">{row.unclear}</td><td className="p-3 font-bold">{row.activeReactions}</td></tr>)}</tbody>
              </table>
            </div>
